...
export default function BrandTab({ brands, setBrands, visitId, token, fetchVisitDetail }: BrandTabProps) {
    ...
    const handleAddBrand = async () => {
        if (newBrand.brandName.trim() !== "") {
            ...
            try {
                const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/visit/addProCons?visitId=${visitId}`, {
                    method: "PUT",
                    headers: {
    ...
    const handleDeleteBrand = async (id: number) => {
        ...
        if (deletedBrand) {
            try {
                const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/visit/deleteProCons?visitId=${visitId}`, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${token}`,
