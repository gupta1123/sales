...
const WorkingDays: React.FC<{ authToken: string | null }> = ({ authToken }) => {
    ...
    const fetchWorkingDays = useCallback(async () => {
        try {
            const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/attendance-rule/getById?id=2`, {
                headers: {
    ...
    const updateWorkingDays = async () => {
        try {
            await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/attendance-rule/edit?id=2`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${authToken}`,
