  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      if (token && id) {
        ...
        try {
          const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/expense/getByEmployeeAndDate?start=${start}&end=${end}&id=${id}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
    ...
    const fetchAttendanceStats = async () => {
      if (token && id) {
        try {
          ...
          const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/attendance-log/monthlyVisits?date=${selectedDate}&employeeId=${id}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
  ...
  useEffect(() => {
    const fetchDailyPricing = async () => {
      if (token && id) {
        ...
        try {
          const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/brand/getByDateRangeForEmployee?start=${start}&end=${end}&id=${id}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
