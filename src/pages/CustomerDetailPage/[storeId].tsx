...
const CustomerDetailPage = () => {
  ...
  const handleCustomerEditSubmit = async (values: any) => {
    try {
      await axios.put(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/store/edit?id=${storeId}`, {
        ...values,
  ...
  const handleAddNote = async () => {
    try {
      const response = await axios.post('http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/notes/create', {
        content: noteContent,
  ...
  const handleSaveEditNote = async () => {
    try {
      await axios.put(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/notes/edit?id=${editingNoteId}`, {
        content: noteContent,
        employeeId: employeeId,
        storeId: parseInt(storeId as string),
      }, {
  ...
  const handleDeleteNote = async (noteId: number) => {
    try {
      await axios.delete(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/notes/delete?id=${noteId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleVisitFormSubmit = async (values: any) => {
    try {
      const response = await axios.post('http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/visit/create', {
        ...values,
        employeeId: employeeId,
        storeId: parseInt(storeId as string),
      }, {
  ...
  const createTask = async () => {
    try {
      const response = await axios.get(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/task/getByStoreAndDate?storeId=${storeId}&start=${format(startDate, 'yyyy-MM-dd')}&end=${format(endDate, 'yyyy-MM-dd')}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleChangeStatus = async (taskId: number, status: string) => {
    try {
      await axios.put(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/task/updateTask?taskId=${taskId}`, {
        status,
        priority: "Medium",
      }, {
        headers: {
