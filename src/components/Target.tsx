...
const TargetComponent: React.FC = () => {
    ...
    const fetchEmployeesByTargetId = async (cityTargetId: number): Promise<EmployeeTarget[]> => {
        try {
            const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/target/employees?cityTargetId=${cityTargetId}`, {
                headers: {
                    'Authorization': `Bearer ${authToken}`
                }
            });
    ...
    const handleContributionChange = async (city: string, employeeId: number, newValue: number) => {
        try {
            ...
            const response = await fetch(`http://ec2-51-20-32-8.eu-north-1.compute.amazonaws.com:8081/target/updateAchievement?employeeTargetId=${employee.employeeTargetId}&achievedValue=${newValue}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${authToken}`
                }
